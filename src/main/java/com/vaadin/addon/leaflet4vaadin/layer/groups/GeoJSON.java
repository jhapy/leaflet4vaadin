...
/**
 * Represents a GeoJSON object or an array of GeoJSON objects. Allows you to parse GeoJSON data and
 * display it on the map. Extends FeatureGroup.
 *
...
public class GeoJSON extends FeatureGroup implements GeoJSONFunctions {
   ...
   *
   * @param geoJson the {@link GeoJSON} to be converted as a {@link Feature}
  ...
  /**
   * Creates a Layer from a given GeoJsonObject object. Can use a custom pointToLayer and/or
   * coordsToLatLng functions if provided as options.
   *
   * @param geoJsonObject the geojson to convert
   * @param options a custom options
   * @return return a {@link Layer} which has been added to this {@link GeoJSON} layer, the type of
   * the added layer depends on type of the given {@link GeoJSON}
   * @see GeoJSONOptions
   */
  public static Layer geometryToLayer(GeoJsonObject geoJsonObject, GeoJSONOptions options) {
    ...
    CoordsToLatLngHandler coordsToLatLng =
        options.coordsToLatLngHandler() != null ? options.coordsToLatLngHandler()
    ...
    if (geoJsonObject instanceof Feature) {
      ...
      MultiLatLngArray latLngs = multiCoordinateToLatLng(multiLineString.getCoordinates(),
          coordsToLatLng);
      ...
      List<LatLng> exteriorLatlngs = polygon.getExteriorRing().stream().map(coordsToLatLng::convert)
          .collect(Collectors.toList());
      MultiLatLngArray interiorLatLngs = multiCoordinateToLatLng(polygon.getInteriorRings(),
          coordsToLatLng);
      ...
      geometryCollection.getGeometries().stream()
          .map((geometry) -> geometryToLayer(geometry, options))
  ...
  private static MultiLatLngArray multiCoordinateToLatLng(List<List<LngLatAlt>> multiLngLatAlts,
    ...
    for (List<LngLatAlt> coords : multiLngLatAlts) {
      List<LatLng> latLngs = coords.stream().map(coordsToLatLng::convert)
          .collect(Collectors.toList());
  ...
  private static Layer pointToLayer(PointToLayerHandler pointToLayerHandler, GeoJsonObject geoJson,
      LatLng latLng) {
