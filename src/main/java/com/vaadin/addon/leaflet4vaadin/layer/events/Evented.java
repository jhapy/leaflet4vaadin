...
/**
 * A set of methods shared between event-powered classes (like Map and Marker). Generally, events
 * allow you to execute some function when something happens with an object (e.g. the user clicks on
 * the map, causing the map to fire 'click' event).
 *
...
public interface Evented {
   ...
   */
  <T extends LeafletEvent> void addEventListener(LeafletEventType eventType,
   ...
   * @param <T> the generic type of the leaflet object
   */
  default <T extends LeafletEvent> void on(LeafletEventType eventType,
      LeafletEventListener<T> listener) {
