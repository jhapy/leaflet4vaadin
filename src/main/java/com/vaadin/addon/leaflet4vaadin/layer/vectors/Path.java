...
/**
 * An abstract class that contains options and constants shared between vector overlays (Polygon,
 * Polyline, Circle). Do not use it directly. Extends InteractiveLayer
 */
public abstract class Path extends InteractiveLayer implements HasStyle, PathFunctions {
  ...
  /**
   * A string that defines the stroke dash pattern. Doesn't work on Canvas-powered layers in some
   * old browsers.
   *
  ...
  /**
   * A string that defines the distance into the dash pattern to start the dash. Doesn't work on
   * Canvas-powered layers in some old browsers.
   *
   * @param dashOffset defines the distance into the dash pattern to start the dash
   */
   ...
   *
   * @param fillRule the string that defines how the inside of a shape is determined
   */
