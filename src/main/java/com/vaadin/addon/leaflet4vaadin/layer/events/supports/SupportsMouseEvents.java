...
public interface SupportsMouseEvents extends Evented {
  ...
  /**
   * Fired when the user right-clicks on the layer, prevents default browser context menu from
   * showing if there are listeners on this event. Also fired on mobile when the user holds a single
   * touch for a second (also called long press).
   *
   * @param listener the listener to call when the event occurs, not {@code null}
   */
