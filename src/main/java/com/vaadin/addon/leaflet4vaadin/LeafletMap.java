...
public final class LeafletMap extends PolymerTemplate<LeafletModel> implements
    MapModifyStateFunctions, MapGetStateFunctions, GeolocationFunctions, MapConversionFunctions,
    SupportsMouseEvents,
    SupportsMapEvents, SupportsLocationEvents, SupportsKeyboardEvents, SupportsLayerEvents, HasSize,
    HasTheme, HasStyle {
  ...
  private void onLayersControlEventHandler(@EventData("event.target.options.uuid") String layerId,
    ...
    Layer layer = findLayer(layerId);
    LeafletEvent leafletEvent = new LayersControlEvent(layer, LayerControlEventType.valueOf(event),
        name);
    fireEvent(layer, leafletEvent);
  }
  ...
  private void onMouseEventEventHandler(@EventData("event.target.options.uuid") String layerId,
      ...
      @EventData("event.latlng.lng") Double longitude,
      @EventData("event.layerPoint.x") int layerPointX,
      @EventData("event.layerPoint.y") int layerPointY,
      @EventData("event.containerPoint.x") int containerPointX,
    ...
    LeafletEvent leafletEvent = new MouseEvent(layer, MouseEventType.valueOf(eventType), latLng,
  ...
  private void onMoveEventHandler(@EventData("event.target.options.uuid") String layerId,
    ...
    LeafletEvent leafletEvent = new MoveEvent(layer, DragEventType.valueOf(eventType), oldLatLng,
        latLng);
    fireEvent(layer, leafletEvent);
  }
  ...
  private void onKeyboardEventHandler(@EventData("target.options.uuid") String layerId,
      ...
      @EventData("event.originalEvent.code") String code,
      @EventData("event.originalEvent.keyCode") int keyCode,
    ...
    Layer layer = findLayer(layerId);
    LeafletEvent leafletEvent = new KeyboardEvent(layer, KeyboardEventType.valueOf(eventType), key,
        code, keyCode, shiftKey, altKey, ctrlKey);
    fireEvent(layer, leafletEvent);
  }
  ...
  private void onLocationEventHandler(@EventData("target.options.uuid") String layerId,
      ...
      @EventData("event.altitude") Double altitude,
      @EventData("event.altitudeAccuracy") Double altitudeAccuracy,
      @EventData("event.heading") Double heading, @EventData("event.speed") Double speed,
      @EventData("event.timestamp") Double timestamp) {
    LatLng latlng = new LatLng(latitude, longitude);
    Layer layer = findLayer(layerId);
    LeafletEvent leafletEvent = new LocationEvent(layer, LocationEventType.valueOf(eventType),
        latlng, null, accuracy, altitude, altitudeAccuracy, heading, speed, timestamp);
    fireEvent(layer, leafletEvent);
  }
  ...
  private void onResizeEventHandler(@EventData("target.options.uuid") String layerId,
    ...
    Layer layer = findLayer(layerId);
    LeafletEvent leafletEvent = new ResizeEvent(layer, Point.of(oldSizeX, oldSizeY),
        Point.of(newSizeX, newSizeY));
    fireEvent(layer, leafletEvent);
  }
  ...
  private void onZoomAnimEventHandler(@EventData("target.options.uuid") String layerId,
    ...
    LeafletEvent event = new ZoomAnimEvent(this.mapLayer, MapEventType.valueOf(eventType), center,
        zoom, false);
  ...
  /**
   * Fired when the map gets initialized with a view (center and zoom) and at least one layer, or
  ...
  public void addThemeVariants(LeafletMapVariant... variants) {
    getThemeNames().addAll(
        Stream.of(variants).map(LeafletMapVariant::getVariantName).collect(Collectors.toList()));
  }
  ...
  @Override
  public <T extends Serializable> CompletableFuture<T> call(Identifiable target,
      String functionName, Class<T> resultType, Serializable... arguments) {
    if (ready) {
      logger.info("Call leaflet function: {}", functionName);
      LeafletOperation leafletOperation = new LeafletOperation(target, functionName, arguments);
      PendingJavaScriptResult javascriptResult = getElement()
          .callJsFunction("callLeafletFunction", JsonSerializer.toJson(leafletOperation));

      CompletableFuture<T> completableFuture = new CompletableFuture<>();
      javascriptResult.then(value -> {
        logger.info("Result of leaflet function: {}", functionName);
        try {
